...
function pis_custom_field( $args ) {
	...
	if ( $args['custom_field_all'] ) {
			...
			foreach ( $the_custom_fields as $cf_key => $cf_value ) {
				...
				if ( '_' !== substr( $cf_key, 0, 1 ) ) {
					foreach ( $cf_value as $k => $cf_v ) {
						...
						$cf_value = '<span ' . pis_class( 'pis-custom-field-value', apply_filters( 'pis_custom_field_span_class', '' ), false ) . '>' . $cf_text_value . '</span>';
		...
		if ( $the_custom_field ) {
			...
			$cf_value = '<span ' . pis_class( 'pis-custom-field-value', apply_filters( 'pis_custom_field_span_class', '' ), false ) . '>' . apply_filters( 'pis_custom_field_value', $cf_text_value ) . '</span>';
